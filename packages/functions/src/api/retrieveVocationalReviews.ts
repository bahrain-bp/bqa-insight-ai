...
// Define the VocationalReview interface
interface VocationalReview {
...
export const handler: APIGatewayProxyHandler = async () => {
  try {
    ...
    if (!tableName) {
      throw new Error("Vocational reviews table name is not defined in environment variables");
    }
    ...
    // Map the DynamoDB items to VocationalReview objects
    const items: VocationalReview[] | undefined = response.Items?.map((item: any) => ({
    ...
    console.error("Error retrieving vocational reviews:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({
        success: false,
