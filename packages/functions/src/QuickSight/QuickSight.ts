...
const detectSchema = (csvData: Record<string, string>[]) => {
  ...
  const inferType = (value: string): string => {
    ...
    // Remove entire citation markers and percentage symbols
    const cleanValue = value.replace(/\s*%\[\d+\]%\s*/g, '').replace('%', '').trim();

    // // Check for numeric
    // if (!isNaN(Number(value))) {
    //   // Differentiate between integer and decimal
  ...
  const columns = Object.keys(csvData[0]).map((columnName) => {
    ...
    // Type selection
    const chosenType = 
      typeCounts["DATETIME"] && typeCounts["DATETIME"] > sampleValues.length / 2 ? "DATETIME" :
      // typeCounts["DECIMAL"] && typeCounts["DECIMAL"] > sampleValues.length / 2 ? "DECIMAL" :
      typeCounts["INTEGER"] && typeCounts["INTEGER"] > sampleValues.length / 2 ? "INTEGER" :
      "STRING";

    console.log(`Column ${columnName} detected as ${chosenType}`);
  ...
  const params = {
    ...
    PhysicalTableMap: {
      [tableId]: {
        S3Source: {
          // data source ARN
          DataSourceArn: `arn:aws:quicksight:us-east-1:588738578192:datasource/${datasetId}`,
          InputColumns: schema.columns.map(col => ({
            Name: col.Name,
            Type:  col.Type === 'INTEGER' ? 'INTEGER' : 
                   'STRING'
...
const createDashboard = async (awsAccountId: string, datasetId: string) => {
  ...
  const params = {
    ...
    SourceEntity: {
      SourceTemplate: {
        // Create template
        Arn: `arn:aws:quicksight:us-east-1:588738578192:analysis/023fc3f6-7963-444e-8be6-63bcaa6acb98`,
        DataSetReferences: [{ DataSetArn: "arn:aws:quicksight:us-east-1:588738578192:dataset/8ce1efc3-1aca-4a23-9c1b-451d4f204f19", DataSetPlaceholder: "testing-quicksight-dashboard-output" }],
      },
    },
    Permissions: [
      {
