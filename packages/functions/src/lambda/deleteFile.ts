  ...
  try {
    ...
    if (!Array.isArray(fileKeys) || fileKeys.length === 0) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          message: "fileKeys must be a non-empty array",
        }),
      };
    }
    ...
    const deletePromises = fileKeys.map(async (fileKey) => {
      try {
        ...
        // Check if the corresponding .txt file exists in S3
        ...
        // Repeat the same process for the metadata.json file
        ...
        if (
          ...
          metadataFileResponse.Contents.some(
            (object) => object.Key === metadataFileKey
          )
        ) {
          // If the file exists, delete it
          await s3
          ...
          // If the file doesn't exist, log a message
          console.log(
            `No corresponding metadata.json file found for key: ${metadataFileKey}`
          );
        }
        ...
        console.log(
          `${fileMetadata.Item?.instituteName} fileMetadata institutename`
        );
        ...
        if (fileMetadata.Item?.instituteName) {
          ...
          await dynamodb
            .delete({ TableName: TABLE_NAME, Key: { fileKey } })
            .promise();
          ...
          if (instituteRecords.Items && instituteRecords.Items.length <= 1) {
            ...
            await dynamodb
              ...
              .promise();
            console.log(
              `Deleted from instititute metadata table: ${fileMetadata.Item.instituteName}`
            );
          } else {
            // There are other files for this institute, so keep the institute record
            const remainingFiles = instituteRecords.Items
              ? instituteRecords.Items.length - 1
              : 0;
            console.log(
              `Keeping institute record as ${remainingFiles} files remain`
            );
          }
        } else {
        ...
        const programScan = await dynamodb
          .scan({
        ...
        if (programScan.Items && programScan.Items.length > 0) {
          ...
          await dynamodb
            ...
            .promise();
          console.log(
            `Deleted from program metadata table: ${program.programmeName}`
          );
        }

        // Check and Delete from University Metadata Table
        const universityScan = await dynamodb
          .scan({
          ...
          // if the university exsits delete it 
        ...
        const vocationalScan = await dynamodb
          ...
          .promise();
          // look for the vocational center it it has items in the table then delete them
        ...
        if (vocationalScan.Items && vocationalScan.Items.length > 0) {
          ...
          await dynamodb
            ...
            .promise();
          console.log(
            `Deleted from vocational center metadata table: ${vocationalCenter.vocationalCenterName}`
          );
        }
      ...
      // called the syncDeleteKnowlegeBase function to sync the delete
      const uri = `s3://${BUCKET_NAME}/${fileKey}`;
      await syncDeleteKnowlegeBase(
        process.env.KNOWLEDGE_BASE_ID || "",
        process.env.DATASOURCE_BASE_ID || "",
        uri
      );
      console.log("Successful sync deleting...");
    });

    // Wait for all deletion promises to finish
    await Promise.all(deletePromises);

    //console log that files are deleted succesfully
    return {
      statusCode: 200,
      body: JSON.stringify({
        success: true,
