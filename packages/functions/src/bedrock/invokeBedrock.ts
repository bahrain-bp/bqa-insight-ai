...
export const invokeBedrockAgent = async (event: APIGatewayEvent) => {
  ...
  try {
    ...
    const command = new InvokeAgentCommand({
      ...
      agentAliasId,
      sessionId,
      inputText: `Goal: To analyze the improvements and declines of individual educational institutes in Bahrain, focusing on specific actions taken, challenges faced, and actionable strategies to enhance their overall performance in areas such as academic achievement, personal development, teaching quality, and governance.
...
Question: What is the performance of ${prompt["Institute Name"]}?
        ...
        const generatedJson = generateJson(decodedResponse)
         ...
         return {
          ...
          body: JSON.stringify({
             ...
             response: generatedJson // Handle the result here
            }),
        };
      } catch (err) {
