//promptfilters.ts: // TODO: change the questions in compare prompt and others
...
export function createAnalyzePrompt(instituteName: string, metric: string): string {
    return `
        ...
        Question: How did ${instituteName} perform in terms of ${metric}?
    `;
}

export function createGenerateJsonPrompt(data: string): string {
    const heading = `Your goal is to analyze the given text and provide a relevant title in relation to the given text in JSON format. 
